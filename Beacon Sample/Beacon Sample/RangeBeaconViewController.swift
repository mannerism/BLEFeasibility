...
class RangeBeaconViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate {
	...
	*/
	let defaultUUID = "6d406fed-5aed-4bc5-a4fe-e5d19682ed0c"
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let apName = getAPName(minor: beacon.minor)
			cell.textLabel?.text = "UUID: \(beacon.uuid.uuidString)"
		cell.detailTextLabel?.text = "Major: \(beacon.major) Location: \(apName)"
		
		return cell
	}
	
	func getAPName(minor:NSNumber) -> String {
		switch minor {
		case 4:
			return "AP5_near_office"
		case 5:
			return "AP6_near_gwangFaOven"
		default:
			return "minor_\(minor)"
		}
	}
